...
export function ApprovalDetail() {
  ...
  if (!approval) return <p className="text-sm text-muted-foreground">{t('approvals.notFound')}</p>;
  ...
  return (
    <div className="space-y-6 max-w-3xl">
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t('approvals.approvalRequestNotified')}
                </p>
              </div>
            </div>
            <Button
        ...
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            ...
            <p className="text-[11px] text-muted-foreground mt-2">
              {t('approvals.linkedIssuesStayOpen')}
            </p>
          </div>
        )}
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t('approvals.resolveBudgetStopFromBefore')}<Link to="/costs" className="underline underline-offset-2">/costs</Link>{t('approvals.resolveBudgetStopFromAfter')}
            </p>
          )}
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            <Button
              ...
              onClick={() => {
                if (!window.confirm(t('approvals.deleteDisapprovedAgentConfirm'))) return;
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">{t('approvals.comments')} ({comments?.length ?? 0})</h3>
