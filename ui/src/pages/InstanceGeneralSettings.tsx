...
export function InstanceGeneralSettings() {
  ...
  const signOutMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("settings.failedToSignOut"));
    },
  });
    ...
    <div className="max-w-4xl space-y-6">
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="space-y-5">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("settings.backupRetention")}</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">
              {t("settings.backupRetentionDescription")}
            </p>
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {DAILY_RETENTION_PRESETS.map((days) => {
                ...
                return (
                  ...
                  >
                    <div className="text-sm font-medium">{days} {t("settings.days")}</div>
                  </button>
                );
              })}
            </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-2">
              {WEEKLY_RETENTION_PRESETS.map((weeks) => {
                ...
                const label = weeks === 1 ? `1 ${t("settings.weeks")}` : `${weeks} ${t("settings.weeks")}`;
                return (
                  <button
                    key={weeks}
                    type="button"
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-2">
              {MONTHLY_RETENTION_PRESETS.map((months) => {
                ...
                const label = months === 1 ? `1 ${t("settings.months")}` : `${months} ${t("settings.months")}`;
                return (
                  <button
                    key={months}
                    type="button"
        ...
        <div className="space-y-4">
          <div className="space-y-1.5">
            ...
            {FEEDBACK_TERMS_URL ? (
              ...
              >
                {t("settings.readOurTermsOfService")}
              </a>
            ) : null}
          </div>
          {feedbackDataSharingPreference === "prompt" ? (
            <div className="rounded-lg border border-border/70 bg-accent/20 px-3 py-2 text-sm text-muted-foreground">
              {t("settings.feedbackNoDefaultDescription")}
            </div>
          ) : null}
          <div className="flex flex-wrap gap-2">
            {[
              ...
              },
              {
                ...
                description: t("settings.dontAllowDescription"),
              },
            ].map((option) => {
